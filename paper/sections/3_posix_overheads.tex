...
data and metadata. Ceph was originally designed for HPC and has proven to be
useful for HPC workloads~\cite{}. Several features were motivated by HPC
requirements~\cite{weil:osdi2006-ceph}, so the lessons are useful (and can be
implemented) in HPC environments.  This file system is an implementation of one
set of design decisions and our goal is to highlight the effect that those
decisions have on performance. During this process we discovered, based on the
analysis and breakdown of costs, that durability and consistency have high
overhead.
