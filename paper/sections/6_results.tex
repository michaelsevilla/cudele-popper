...
Clients creating files in private directories is heavily
studied in HPC~\cite{plfs, cephfs04, indexfs, giga+, batchfs, deltafs,
sevilla:eurosys, shardfs, mantle}, mostly due to checkpoint-restart~\cite{}.
...
complete~\cite{}. A more familiar example is uncompressing an archive ({\it
e.g.}, \texttt{tar xzf}), where the file system services a flash crowd of
creates across all directories as shown in Figure~\ref{fig:overhead-creates}.
We use this as a microbenchmark because it allows us to control the size of the
log, since each create creates a single journal event.
...
% up to 30 clients
\subsubsection{Number of Serialized Merges}
...
Cudele serializes merge requests at the metadata server before applying the
...
worklflows~\cite{batchfs, deltafs}, it stills happens in distributed file
systems when users unintentionally access directories in a shared file system.
