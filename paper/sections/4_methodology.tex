...
mechanisms used by default in CephFS.  Using existing configuration settings we
can turn Stream on and off.  If it is off, then the metadata servers will not
save journals in the object store. For \underline{Local Persist}, clients write
serialized log events to a file on local disk and for \underline{Global
Persist}, clients push the journal into the object store. The overheads for
both Local Persist and Global Persist is the write bandwidth of the local disk
and object store, respectively.  These persist mechanisms are part of the
...
number of inodes to provision to the decoupled namespace (100), and which interfere
...
decoupled namespace ({\it i.e.} at any depth in the subtree).  ``Interfere Policy"
