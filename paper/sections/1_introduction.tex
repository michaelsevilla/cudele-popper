...
consistency because it  does not guarantee that all updates are not immediately
seen by all servers; the BatchFS and RAMDisk subtrees are decoupled from the
global namespace and have similar consistency/durability behavior to those
systems; and the POSIX subtree retains the rigidity of POSIX's strong
consistency.
...
file format in systems that are based on in-memory data structures.  Our
prototype is built on Ceph because it was originally designed for HPC and has
proven to be useful for HPC workloads~\cite{}. Several features were motivated
by HPC requirements~\cite{weil:osdi2006-ceph}, so the lessons are useful (and
can be implemented) in HPC environments. More importantly,  we use Ceph as a
