...
% What is the problem?
File system metadata services in HPC and data center applications  have
scalability problems because administrative tasks, like
...
by current trends; for example, HPC architectures are transitioning from
complex storage stacks with burst buffer, file system, object store, and tape
tiers to more simplified stacks with just a burst buffer and object
store~\cite{bent:login16-hpc-trends}. These types of trends put more pressure
on data access because more requests end up hitting the same layer and
