...
guarantees to subtrees in the global namespace, which lets administrators
custom fit semantics to application. We show how applications can co-exist and
...
Cudele prompts many avenues for future work.  First is dynamically changing
semantics of a subtree from stronger to weaker guarantees (or vice versa).  For
example, the results of a Hadoop job do not need to be migrated into CephFS for
...
metadata/data movement to make ensure strong consistency. Second is embeddable
...
IO subtree that is strongly consistent.  
