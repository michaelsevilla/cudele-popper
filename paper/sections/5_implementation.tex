...
metadata store to service requests. Using code designed for recovery,
Nonvolatile Apply and Volatile Apply replay updates onto the metadata store in
memory and in the object store, respectively.  When the clients are ready to
...
bytes~\cite{docs:cephinternalds}.  The inode cache has code for manipulating
...
from each other. \textbf{In Cudele} the large inodes store consistency and
