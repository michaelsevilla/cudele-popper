...
\textbf{Metadata Store}: In CephFS, the metadata store is a data structure that represents the file
...
\textbf{Journal}: The journal is the second way that CephFS represents the file system namespace;
...
\textbf{Journal Tool}: The journal tool is used for disaster recovery and lets administrators view and
...
\textbf{Inode Cache}: In CephFS, the inode cache reduces the number of RPCs between clients and
...
\textbf{Large Inodes}: In CephFS, inodes already store policies, like how the file is striped
