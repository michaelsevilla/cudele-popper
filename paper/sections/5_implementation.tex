...
% how i used it
\noindent\textbf{Cudele}: the ``RPCs" mechanism uses the metadata store to
...
\noindent\textbf{Cudele}: the journal format is used by ``Stream", ``Append Client Journal",
...
\noindent\textbf{Cudele}: the external library the clients link into is based
...
\noindent\textbf{Cudele}: ``Nonvolatile Apply" uses the internal inode cache
